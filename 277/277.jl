...
# ╔═╡ c430402a-e51d-11ec-2142-3bc222c479ce
using Symbolics

# ╔═╡ 9a0c8d4a-f4fa-4bbb-a251-98d16ef2a62a
using Primes

# ╔═╡ 61b12468-4b79-42c3-910c-c0ad97eb9bd4
let seq = "DdDddUUdDD"
	function udd()
		s = "x"
		for f in seq
...
# ╔═╡ 0e91c082-45c3-42f0-9790-cf872c6c8a5e
let seq = "UDDDUdddDDUDDddDdDddDDUDDdUUDd"
	s = "x"
	for f in seq
		s = f * "($s)"
	end
...
# ╔═╡ efc70699-e085-471c-995e-1f15b4d2be54
let seq = "UDDDUdddDDUDDddDdDddDDUDDdUUDd"
	@show length(seq)
	@show count(i->(i=='U'), seq)
	@show count(i->(i=='d'), seq)
	2^22, 3^30
end

# ╔═╡ 2bfcf9c7-1bab-49a0-a50a-e2478898523e
let
	m = 21110037246199    # x = (3^30i + m) / 2^22
	mm = m % 2^22
	p = 3^30 % 2^22
	pn = gcdx(p, 2^22)[2]    # inverse of p mod 2^22
	im = BigInt(pn * (-m) % 2^22)    # i ≡ im (mod 2^22)
	i = im
	while (3^30 * i + m) ÷ 2^22 < 10^15
		i += 2^22
	end
	(3^30 * i + m) ÷ 2^22
end

# ╔═╡ 8d17dd1d-668f-4e0d-8caa-6ad9c3ea315b
3^30

# ╔═╡ 8e3302d9-ca42-40c6-8688-348e96360aff
s  = :x

# ╔═╡ 7758b8a3-a355-4633-995a-b13b9f58a658
"D($s)"

# ╔═╡ 7652442b-5752-48a9-b607-569e83622d87
Meta.parse("D(3)")

# ╔═╡ d3d638c3-eb3a-4a89-95af-7e7ae90b2d96
eval(:(D(3)))

# ╔═╡ cade6624-1702-4d1b-af6f-7811b2240044
59049 ÷ 19683

# ╔═╡ a2998819-6208-458e-9690-0a9d7bb7d136
59049 % 256

# ╔═╡ 7dad1da9-f0ec-47bd-aece-c0e829487593
(59049 * (256*17+ 1) + 87)÷256

# ╔═╡ 22660d5f-c1e6-4fa5-a465-fc66bc010a8b
4353 ÷ 256

# ╔═╡ 23ad1fca-f2ee-4fa4-9af3-65ce293d0953
begin
	@variables i
	(59049i + 87)/256
end

# ╔═╡ 6d8c5440-54c2-481e-8fbf-40ef041e5c92
(1004064 * 256 - 87) / 59049

# ╔═╡ a313e414-604e-48df-a2ce-5eea0632f6e8
factor(59049)

# ╔═╡ b0fb7ead-773b-4b08-892a-3e335a3367d5
factor(256)

# ╔═╡ d7fbfbd4-72d6-4059-8676-01a15d95c7da
D(x) = x/3

# ╔═╡ 60b59856-3ef0-4cc3-85a4-869f6364e000
d(x) = (2x-1)/3

# ╔═╡ a8d57091-eabb-4f0f-a439-c1ec5386274b
U(x) = (4x+2)/3

# ╔═╡ 786fdeba-224e-4d53-91ac-de79db034d93
let
	@variables x
	d(D(U(U(d(D(D(U(D(D(d(d(D(d(D(d(d(D(D(U(D(D(d(d(d(U(D(D(D(U(x))))))))))))))))))))))))))))))
end

# ╔═╡ fe64c1fc-9c5e-4be1-8b66-311f0a796a60
let
	@variables x
	x |> D |> d |> D |> d |> d |> U |> U |> d |> D |> D
end
